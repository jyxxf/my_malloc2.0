...
#define is_legal(x) ((void *)(x) < (void *)memory + sizeof(memory))
...
void *my_malloc(size_t length)
{
    ...
    while (!check_this_struct(p, length))
    {
        previous_p = p;
        p = (void *)p + p->after;
        if ((void *)p >= (void *)memory + MEMORY_LENGTH)
            return 0; // 满了
    }
    ...
    later_p = (void *)p + p->after;
    insert_p = (void *)p + sizeof(struct _malloc_struct) + length; // 创建一个later结构malloc数据结构
    if (insert_p != later_p)
    {
        //可以容得下insert_p
    ...
    return (void *)p + sizeof(struct _malloc_struct);
}

void my_free(void *p)
{
    struct _malloc_struct *s = p - sizeof(struct _malloc_struct);
    struct _malloc_struct *previous = (void *)s - s->before, *later = (void *)s + s->after;
    s->is_malloc = 0;
    if (is_legal(later) && !later->is_malloc) // later存在且空
    {
        s->after += later->after;
        later = (void *)s + s->after;
        if (is_legal(later))
...
u16 whether_can_malloc_the_size(u16 length)
{
    ...
    while ((void *)p - (void *)memory < MEMORY_LENGTH)
    {
        if (!p->is_malloc)
        {
